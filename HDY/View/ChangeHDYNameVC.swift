...
class ChangeHDYNameVC: RootVC   {
    ...
    func toggleRightMenu(sender: AnyObject)
    {
        ...
        self.httpPostApi(AppConfig.Url_ChangeHDYName, body: parameter, tag: 11)
    }
    ...
    {
        self.title="更换活动邮号";
        self.txtName.text = AppConfig.sharedAppConfig.HDYName
    ...
    override func requestDataComplete(response:AnyObject,tag:Int)
    {
        if (tag == 11)
        {
            AppConfig.sharedAppConfig.HDYName = response as! String
